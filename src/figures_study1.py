...
def plot_within_subtype(df, proxy_dist, out_path: Path):
    subtypes = ["ADHD-I", "ADHD-HI"]
    genders = ["MEN", "WOMEN"]
...
if __name__ == "__main__":
    df_imp = load_data(
        RESPONSES_DIR / "s1_im_responses.csv"
    )

    df_exp = load_data(
        RESPONSES_DIR / "s1_ex_responses.csv"
    )
    ...
    plot_2x2(
        ...
        df_imp,
        PROXY_GENDER_DIST,
        out_path=FIGURE_DIR / "figure_main_s1.png"
    )

    plot_within_subtype(
        df_imp,
        PROXY_GENDER_DIST,
        out_path=FIGURE_DIR / "figure_s1_implicit.png"
    )

    plot_within_subtype(
        df_exp,
        PROXY_GENDER_DIST,
        out_path=FIGURE_DIR / "figure_s1_explicit.png"
    )